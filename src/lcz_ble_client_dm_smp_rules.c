/**
 * @file lcz_ble_client_dm_dmp_rules.c
 * @brief Holds rules for SMP access for the DM client
 *
...
LOG_MODULE_REGISTER(lcz_ble_client_dm_smp_rules, CONFIG_LCZ_BLE_CLIENT_DM_LOG_LEVEL);
...
#endif
static bool client_dm_smp_test(uint16_t group_id, uint16_t command_id);
static int lcz_ble_client_dm_smp_rules_init(const struct device *device);
...
static void reset_auth_timer(void)
{
	k_timeout_t auth_timeout = K_SECONDS(CONFIG_LCZ_CLIENT_DM_SMP_AUTH_TIMEOUT);

#if defined(ATTR_ID_smp_auth_timeout)
	auth_timeout = K_SECONDS(
		attr_get_uint32(ATTR_ID_smp_auth_timeout, CONFIG_LCZ_CLIENT_DM_SMP_AUTH_TIMEOUT));
#endif
...
/* SYS INIT                                                                                       */
/**************************************************************************************************/
SYS_INIT(lcz_ble_client_dm_smp_rules_init, APPLICATION, CONFIG_LCZ_CLIENT_DM_SMP_RULES_INIT_PRIORITY);
static int lcz_ble_client_dm_smp_rules_init(const struct device *device)
{
	/* Register our rules function with the mgmt layer */
	mgmt_register_permission_cb(client_dm_smp_test);
